  ...
  return (
    <RoundView style={{padding: 15}}>
      <View style={[AppStyles.row, {justifyContent: 'space-between'}]}>
        ...
        <Item
          value={isUse ? '占用中' : '未占用'}
          backgroundColor={AppUtil.getDeviceUsingColor(isUse)}
        />
        <Item
          value={`${temperature}°`}
          title={'温度'}
          backgroundColor={AppUtil.getTemperatureColor(temperature)}
        />
      </View>
    </RoundView>
  );
...
const Item = ({title, value, backgroundColor}:
                { title?: string, value: string | number, backgroundColor: ColorValue }) => (
  <View style={[AppStyles.column, styles.itemContainer, {backgroundColor: backgroundColor}]}>
